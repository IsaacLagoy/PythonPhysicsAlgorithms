...
class SkeletonHandler():
    ...
    def create_joint(self, joint_type:str, parent_offset:glm.vec3, child_offset:glm.vec3, spring_constant:float=1e3, min_radius:float=0, max_radius:float=1):
        
        match joint_type:
            case 'free' : return Joint(parent_offset, child_offset, spring_constant, min_radius, max_radius)
            case 'ball' : return Joint(parent_offset, child_offset, spring_constant, min_radius, max_radius)
...
class Bone():
    ...
    def restrict_bones(self, delta_time:float) -> None:
        ...
        for joint in self.bones.values():
            joint.rotate_parent_offset(rotation)
